...
function ResetPasswordForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (!email) { setError("Invalid or expired reset link. Please request a new one."); return; }
    if (!code || !password || !confirm) { setError("All fields are required"); return; }
    ...
    try {
      const endpoint = isSuperAdmin
        ? `${API_BASE}/api/superadmin/reset-password`
        : `${API_BASE}/api/auth/reset-password`;

      const res  = await fetch(endpoint, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, code: code.trim(), password: password.trim() }),
      });
        ...
        <main className={styles.mainContent}>
          <div className={styles.formCard}>
            ...
            <button className={styles.submitBtn} onClick={handleSubmit} disabled={loading}>
              {loading ? <><span className={styles.btnSpinner} /> Updating…</> : "Update Password"}
            </button>

            <button className={styles.resendBtn} onClick={resendCode} disabled={seconds > 0}>
              Resend Reset Code
            </button>
