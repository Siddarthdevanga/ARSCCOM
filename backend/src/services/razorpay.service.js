...
/* ======================================================
   TRIAL — ONE-TIME ₹49 ORDER, NO RECURRING MANDATE.
   --------------------------------------------------------
   Landing-page popup only (Path A, no account yet). A trial customer
   from this path pays once and is never charged again automatically —
   after their 15 days they must manually pick a plan (Business/
   Enterprise) from Settings/Plans, same as before the mandate-based
   auto-convert experiment. That experiment (a real Subscription with a
   delayed-start mandate) is still used for Path B — a web-registered
   company choosing Trial from the in-app /subscription page — see
   razorpayTrialSubscription.service.js. Deliberately not unified: only
   a customer who explicitly chooses a paid plan should ever get a
   recurring mandate.
====================================================== */
...
export const createTrialOrder = async ({ email, phone }) => {
  ...
  // phone as "91" + 10 digits, while this popup only ever collects the
  // bare 10-digit number. A raw match would miss a duplicate against a
  // web-registered account that used this same phone with a different
  // email.
...
const extractPayerDetails = async (paymentEntity) => {
  ...
  // The Razorpay account also takes other, unrelated payments (Business/
  // Enterprise subscriptions, Path B trial mandates) — without this tag
...
export const handlePaymentCaptured = async (payload) => {
  ...
  // account (it also takes Business/Enterprise subscription charges and
  // Path B trial mandates). Never create an account for this, no matter
  // what email/phone/amount happens to be on it.
  if (!isOurOrder) {
    console.warn(
      "[RAZORPAY] payment.captured for a non-trial-order payment, ignoring:",
  ...
  const [[existingUser]] = await db.query(
    `SELECT u.id, u.email, u.phone, c.name AS companyName
     FROM users u JOIN companies c ON c.id = u.company_id
     WHERE u.email = ? OR RIGHT(u.phone, 10) = ? LIMIT 1`,
    [email, phone]
  );
