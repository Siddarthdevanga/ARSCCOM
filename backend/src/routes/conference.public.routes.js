  ...
  try {
    ...
    await db.query(
      `INSERT INTO conference_bookings
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'BOOKED')`,
      [
        company.id, room_id, email, department,
        purpose.trim(), booking_date, start_time, end_time
      ]
    );

    await sendEmail({
      to: email,
      subject: "Conference Booking Confirmed",
      html: `
        <h3>Booking Confirmed</h3>
        <p>${company.name}</p>
        <p>${booking_date} — ${start_time} to ${end_time}</p>
      `
    });
...
router.patch("/company/:slug/bookings/:id", async (req, res) => {
  try {
    ...
    await sendEmail({
      to: normalizedEmail,
      subject: "Booking Time Updated",
      html: `
        <h3>Booking Updated</h3>
        <p>${company.name}</p>
        <p>Date: ${booking.booking_date}</p>
        <p>New Time: ${start_time} – ${end_time}</p>
      `
    });
